...
/**
 * Koreguota 2018-09-29 by 
 * Giedrius Kristinaitis (realizuota add(int index, E element), addFirst(E element), 
 *   			lastIndexOf(Object o), retainAll(ListKTU<?> c), remove(int index), set(int index, E element))
 *
 *
 * @author Aleksis
...
public class ListKTU<E extends Comparable<E>>
	 ...
	 * @return index of the last occurence of the object, -1 if it doesn't exist
	 */
	public int lastIndexOf(Object o){
		int index = -1;
		int k = 0;

		for(Node node = first; node != null; node = node.next, k++){
	 ...
	 * @param c collection with elements to retain
	 * @return true if this collection has been modified, false otherwise
	 */
	public boolean retainAll(ListKTU<?> c){
		if(size == 0){
			return false;
		}
		...
		for(Node<E> node = first; node != null; node = node.next, index++){
			...
			for(Iterator<?> iter = c.iterator(); iter.hasNext();){
				if(node.element.equals(iter.next())){
					exists = true;
					break;
				}
			}
